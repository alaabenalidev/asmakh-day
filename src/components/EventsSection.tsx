...
const getNextLastThursdays = () => {
    ...
    for (let i = 0; i < 6; i++) {
        ...
        const dayOfWeek = lastDay.getDay();
        const diff = (dayOfWeek + 7 - 3) % 7; // Tuesday = 2
        lastThursday.setDate(lastDay.getDate() - diff);
...
const formatGoogleDate = (date: Date, hour: number) => {
    const d = new Date(date);
    d.setHours(hour, 0, 0, 0);
    return d.toISOString().replace(/[-:]/g, "").split(".")[0] + "Z";
};

const buildAppleCalendarUrl = (date: Date) => {
    const isoDate = date.toISOString().split("T")[0];
    return `/api/calendar?date=${isoDate}`;
};

const buildGoogleCalendarUrl = (date: Date) => {
    const title = `Asmakh Day ${date.toLocaleDateString("en-US")}`;
    const start = formatGoogleDate(date, 12); // 12:00 AM
    const end = formatGoogleDate(date, 14);   // 2:00 PM

    return `https://www.google.com/calendar/render?action=TEMPLATE
&text=${encodeURIComponent(title)}
&location=${encodeURIComponent("Al Asmakh Real Estate HQ")}
&dates=${start}/${end}`;
};

const buildWebcalUrl = (date: Date) => {
    const isoDate = date.toISOString().split("T")[0];
    return `webcal://yourdomain.com/api/calendar?date=${isoDate}`;
};

const handleSetReminder = (date: Date) => {
    if (isAppleDevice()) {
        // window.location.href = buildWebcalUrl(date);
        window.location.href = buildAppleCalendarUrl(date);
    } else {
        window.open(buildGoogleCalendarUrl(date), "_blank");
    }
};
        ...
        <section id="events" className="py-20 md:py-32">
          ...
          <span className="inline-block bg-primary/10 text-primary px-4 py-2 rounded-full text-sm font-medium mb-4">
            Event Calendar
                    ...
                    <h2 className="font-display text-3xl md:text-4xl lg:text-7xl font-bold mb-6">
                        Upcoming <span className="text-secondary">Journey Dates</span>
                    </h2>
                    <p className="text-lg text-muted-foreground">
                        Mark your calendars! Join us every last Tuesday of the month.
                    </p>
                </motion.div>
                      ...
                      {event.day-1}
                                ...
                                <Button variant="outline-glass" className="p-0 m-0 mt-4 border-none">
                                    <a
                                        ...
                                        onClick={(e) => {
                                            e.preventDefault();
                                            handleSetReminder(event.date);
                                        }}
                                        className={`button-animation cta ${
                                            event.isNext
                                                ? "bg-[#6225E6]"
